...
export function Preloader() {
  ...
  useEffect(() => {
    // Same logic as ThemeToggle - default is "dark"
  ...
  const bgColor = isLight ? '#ffffff' : '#132337';
  const racketColor = isLight ? '#FB312E' : '#FCC054';
  const ballColor = isLight ? '#FCC054' : '#FFFFFF';
    ...
    >
      <style>
        {`
          ...
          .racket-reveal {
            animation: revealFromTop 0.7s ease-out forwards;
          }
      ...
      <div className="relative flex flex-col items-center">
        ...
        >
          <g>
            {/* Racket - reveals from top */}
            <path
              d={svgPaths.p32e07400}
              fill={racketColor}
              className="racket-reveal"
            />

            {/* Ball - pops in */}
            <path
              d={svgPaths.p3f69f680}
              fill={ballColor}
              className="ball-pop"
            />
          </g>
