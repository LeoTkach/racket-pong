...
async function updatePaperChampionDescription() {
    try {
        console.log('Updating "Paper Champion" description...');

        // Update achievement description
        const result = await pool.query(`
      UPDATE achievements 
      SET description = 'Reach the tournament final but lose'
      WHERE name = 'Paper Champion'
    ...
    `);
        ...
        if (result.rows.length > 0) {
            ...
            console.log('⚠️  No achievement found with name "Paper Champion"');
        }
...
// Run migration
updatePaperChampionDescription();
